...
PMDresult Admittance2(PMDPeriphHandle* hPeriphSer, PMDAxisHandle* hAxis1, PMDPeriphHandle* hAnlgPeriph) {
	...
	int run = 01, save = 01, fullArray = 0, firstRun = 1;
	PMDuint32 binSave = 0b00101111;

	int count = 0, countInput = comConfig._rep, i = 0, dir = 1;
	...
	// Array offsets
	int pOffset = 10000, vOffset = 10000, fOffset = 1000000000, mOffset = 15000;
	...
	PMDint16 motCom[1] = { 0 };
	PMDint32 DesVeL[arraySize] = { vOffset };
	...
	while (run) {
		...
		if ((float)(0.001 * (curTime - cycTime) * sampleTime) >= timeMS) {
			...
			force[i] = 00 + fOffset;
			DesVeL[i] = round(dir * (double)((double)(velGet / velScale) / sampleTime) * 1000) + vOffset;
			if (fullArray == 0) {
				SendParts(hPeriphSer, binSave, TIME[i], POS[i], VeL[i], force[i], motCom[i], DesVeL[i]);
			}
			i++;
		...
		if ((float)(0.001 * (curTime - serTime) * sampleTime) >= serTimeMS) {
			...
			if (comConfig._mode != 0x02 && comConfig._mode != 0x07) {
				run = 0;
			}
