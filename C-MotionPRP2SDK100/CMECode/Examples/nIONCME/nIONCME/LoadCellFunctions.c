...
LoadCellConfig loadCellConfig = { 119040, 205.184006 };  // Initialize offset and scaleFactor
...
PMDint32 read(PMDPeriphHandle* hPeriphLoad) {
	...
	PMD_RESULT(PMDPeriphSend(hPeriphLoad, &clockDATA, 2, 100));
	PMD_RESULT(PMDPeriphReceive(hPeriphLoad, &TEMP, &buffer, sizeof(TEMP), 100));
